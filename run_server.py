...
def check_prerequisites():
    ...
    print(f"[CONFIG] Mode: {'CSV Fallback' if USE_CSV else 'PostgreSQL Database'}")
    print(f"[DIR] Checking structure...\n")
    ...
    if backend_upper.exists() and not backend_dir.exists():
        issues.append(
            "[ERROR] Folder named 'Backend' found but should be 'backend' (lowercase)\n"
    ...
    if not backend_dir.exists():
        issues.append("[ERROR] backend folder not found!")
        return issues
    ...
    if not env_path.exists():
        ...
        if env_txt.exists():
            issues.append(
                f"[WARN]  .env file not found. Copy .env.txt to .env:\n"
                f"   copy backend\\.env.txt backend\\.env"
            )
        else:
            issues.append("[ERROR] Neither .env nor .env.txt found in backend/")
    else:
    ...
    if USE_CSV:
        print("[DATA] CSV Mode enabled - checking for Data folder...")
        data_dir = project_root / "Data"
        if not data_dir.exists():
            data_dir = project_root / "data"
        
        if not data_dir.exists():
            issues.append("[ERROR] Data folder not found! Should be 'Data' or 'data'")
        else:
            required_csvs = [
            ...
            if missing_csvs:
                issues.append(f"[ERROR] Missing CSV files: {', '.join(missing_csvs)}")
            else:
                print("[OK] All required CSV files found")
    else:
        print("[OK] Using PostgreSQL database (CSV fallback disabled)")
        print("   Data folder not required")
    
    # Check backend/__init__.py
    init_file = backend_dir / "__init__.py"
    if not init_file.exists():
        print("[WARN]  backend/__init__.py missing (creating it...)")
        try:
            init_file.touch()
            print("   [OK] Created backend/__init__.py")
        except Exception as e:
            issues.append(f"   [ERROR] Failed to create __init__.py: {e}")
    else:
        print("[OK] backend/__init__.py exists")
    
    return issues

def main():
    print("[TCDAFS] TCDAFS Backend Server Startup")
    print("=" * 50)
    ...
    if issues:
        ...
        # Only fail on critical errors ([ERROR]), not warnings ([WARN])
        critical_errors = [issue for issue in issues if "[ERROR]" in issue]
        
        if critical_errors:
            print("\n[ERROR] Cannot start server. Fix the critical issues ([ERROR]) above first.")
            print("\nQuick fix commands:")
            ...
            print("\n[WARN]  Warnings present, but attempting to start...")
    else:
    ...
    print("=" * 50)
    print("[SERVER] Server will be at: http://localhost:8000")
    ...
    try:
        ...
        print("\n[ERROR] uvicorn not installed. Install dependencies:")
