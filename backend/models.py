...
Version 2.1: Added AI prediction models (compartment_predictions, prediction_logs)
"""
from sqlalchemy import (
    Column, Integer, String, Date, DateTime, Boolean,
    ForeignKey, DECIMAL, Text, Time, Enum as SQLEnum,
    CheckConstraint, Index, Interval, Float, JSON
)
...
class TrainModel(Base):
    ...
    # Route assignments (boolean columns for R01-R09)
    r01 = Column(Boolean, default=False)
    r02 = Column(Boolean, default=False)
    r03 = Column(Boolean, default=False)
    r04 = Column(Boolean, default=False)
    r05 = Column(Boolean, default=False)
    r06 = Column(Boolean, default=False)
    r07 = Column(Boolean, default=False)
    r08 = Column(Boolean, default=False)
    r09 = Column(Boolean, default=False)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
...
# =====================================================
# AI-POWERED PREDICTIONS (New in v2.1)
# =====================================================
...
class CompartmentPrediction(Base):
    ...
    __table_args__ = (
        Index('idx_predictions_schedule_date', 'schedule_id', 'schedule_date'),
        Index('idx_predictions_active', 'is_active'),
    )
...
class PredictionLog(Base):
    ...
    __table_args__ = (
        Index('idx_logs_schedule', 'schedule_id'),
